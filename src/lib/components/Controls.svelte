<script lang="ts">
  ...
  import { camera, brightness, exposureValue } from '$lib/stores/camera';
  ...
  <Section title="Meter">
      <div class="grid grid-cols-2 gap-4">
          <div class="p-3 bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800">
              ...
              <div class="text-lg font-mono">{$exposureValue}</div>
          </div>
          <div class="p-3 bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800">
              <div class="text-[10px] text-zinc-500 mb-1 font-mono">BRIGHTNESS</div>
              <div class="text-lg font-mono">{Math.round($brightness * 100)}%</div>
          </div>
      </div>
  </Section>
</div>
